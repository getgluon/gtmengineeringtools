...
export default async function handler(req, res) {
  ...
  try {
    sendEvent(res, "status", { message: "Initializing deep research agent..." });

    const result = await runResearch(company, (msg) => sendEvent(res, "status", { message: msg }));

    if (!result) {
      sendEvent(res, "error", { message: "Could not generate research brief. Please try again." });
    } else {
